...
import json, sys, re, os, shutil, platform, commands, time, getopt, codecs
...
# 平文本类型扩展名
plaintext = ["js", "css", "html", "htm", "manifest"]
...
class Compile(object):
    ...
    def createFile(self, tpath, contents):
        ...
        if self.markreplace:
            output = self.replace(tpath, "".join([l for l in contents]).decode('utf-8'))
        else:
...
def parseMarks():
    ...
    marks['%Version%'] = REV
    marks['%TimeStamp%'] = TIMESTAMP
    marks = marks.copy()
    marks.update(cmarks)

def sortRules(x, y):
    return cmp(y['v'].get('level', 0), x['v'].get('level', 0))
...
if __name__ == '__main__':
    ...
    modeRules = modes.get(mode);
    if not modeRules:
        tmpRules = [{'k':key, 'v': value} for key, value in rules.iteritems()]
        tmpRules.sort(sortRules)
        modeRules = [item.get('k') for item in tmpRules]
