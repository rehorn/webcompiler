...
Jx().$package(function(J){
    ...
    */
    ...
    timedChunk = function(items, process, context, isShift, callback) {
		...
		window.setTimeout(function() {
            ...
            if(todo.length > 0) {
                timeOut = window.setTimeout(arguments.callee, delay);
            } else if(callback) {
                callback(items);
            }
    ...
    */
    ...
    var Chunk = new Class({
        init : function(items, process, context, isShift, callback, stopCallback) {
        	var cxt = this;
        	cxt.isRunning = true;
            var todo = items.concat(), delay = 25;
            if(isShift){
            ...
            this.timeout;
             this.timeout = window.setTimeout(function() {
                ...
                while(todo.length > 0 && (+new Date() - start < 50) && cxt.isRunning) {
                    process.call(context, todo.shift());
                };
     
                if(todo.length > 0 && cxt.isRunning) {
                    this.timeout = window.setTimeout(arguments.callee, delay);
                }else if(!cxt.isRunning){
                	callback(items, 'stop');
                }else if(callback) {
                    callback(items);
                }
